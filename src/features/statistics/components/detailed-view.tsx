          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-white/90 truncate">{habit.name}</p>
      ...
      {data.goals.length === 0 ? (
        ...
        data.goals.map((goal, i) => (
          ...
          >
            ...
            />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-white/90 truncate">{goal.name}</p>
              <div className="flex items-center gap-3 mt-1 text-[11px] text-white/35">
            ...
            <span className="text-sm font-bold text-emerald-400 shrink-0">{goal.progress}%</span>
          </motion.div>
        ))
      )}
    </Panel>
          ...
          <div className="space-y-1.5 max-h-[280px] overflow-y-auto custom-scrollbar pr-1">
            {data.weeks
              ...
              .map((w, i) => (
                ...
                >
                  <span className="text-white/25 w-24 shrink-0 font-mono text-[11px]">{w.weekKey}</span>
...
const StatBox: React.FC<{ label: string; value: string; sub?: string }> = ({ label, value, sub }) => (
  <div className="rounded-2xl bg-white/[0.02] border border-white/5 p-3 text-center">
    <p className="text-[10px] uppercase tracking-wider text-white/30 font-bold mb-1">{label}</p>
    <p className="text-lg font-black text-white/70">{value}</p>
