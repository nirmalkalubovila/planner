import * as React from 'react';
import { Pressable, type PressableProps } from 'react-native';
import Svg, { Path } from 'react-native-svg';
import { cn } from '@/lib/cn';
import { Text } from './typography';

/* The "G" mark, paths copied 1:1 from apps/web/src/components/ui/auth-layout.tsx
   so the two apps render an identical logo. */
const GoogleMark: React.FC = () => (
  <Svg width={16} height={16} viewBox="0 0 24 24">
    <Path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
    <Path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.16v2.84C3.99 20.53 7.7 23 12 23z" />
    <Path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.16C1.43 8.55 1 10.22 1 12s.43 3.45 1.16 4.93l3.68-2.84z" />
    <Path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.16 7.07l3.68 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
  </Svg>
);

export interface GoogleButtonProps extends PressableProps {
  label?: string;
}

export const GoogleButton = React.forwardRef<React.ElementRef<typeof Pressable>, GoogleButtonProps>(
  ({ label = 'Sign in with Google', className, ...props }, ref) => (
    <Pressable
      ref={ref}
      className={cn(
        'w-full flex-row items-center justify-center gap-2 h-9 px-4 rounded-md border border-input bg-background active:bg-accent',
        className
      )}
      {...props}
    >
      <GoogleMark />
      <Text className="text-sm font-medium text-foreground">{label}</Text>
    </Pressable>
  )
);
GoogleButton.displayName = 'GoogleButton';
