...
import { toast, type Goal, type AIGeneratedPlanSlot } from '@llb/core';
import { supabase, useUserProfile } from '@llb/api';
...
export async function callAI(prompt: string): Promise<AIGeneratedPlanSlot[]> {
  const { data, error } = await supabase.functions.invoke('generate-ai-plan', { body: { prompt } });
  if (error) throw new Error(error.message || 'AI Generation Edge Function error');
  if (!data) throw new Error('AI Generation Edge Function returned empty response');
  return data as AIGeneratedPlanSlot[];
}

/** Port of apps/web/src/features/goals/hooks/use-ai-plan-generation.ts — the
 * edge function call (`generate-ai-plan`) is platform-agnostic and ports
 * as-is; only the localStorage-based gen-time tracking (in
 * ai-loading-popup.tsx) needed swapping to MMKV. */
export function useAiPlanGeneration(user: User | null) {
